...
const getProductData = async (productId: any) => {
...
export const POST =async (request:NextRequest) => {
    ...
    if(!user_id)
    {
        setCookies.set("user_id", user_id);
    }
...
export const DELETE =async (request:NextRequest) => {
    ...
    try{
        ...
        if(user_id && req.product_id){
            ...
            const res = await db.delete(cartTable).where(eq(cartTable.user_id,user_id) && eq(cartTable.product_id,req.product_id)).returning(); 
...
export const PUT =async (request:NextRequest) => {
    ...
    try{
        ...
        if(user_id && req.product_id){
            ...
            const res = await db.update(cartTable).set({quantity:req.quantity}).where(and(eq(cartTable.user_id,user_id), eq(cartTable.product_id,req.product_id))).returning(); 
...
export const PATCH =async (request:NextRequest) => {
    ...
    try{
        ...
        if(!user_id && req.product_id){
            ...
            const res = await db.update(cartTable).set({quantity:req.quantity}).where(eq(cartTable.user_id,user_id) && eq(cartTable.product_id,req.product_id)).returning(); 
